...
async function mintThenList (URI, collection) {
  ...
  await(await nft.connect(deployer).setApprovalForAll(store.address, true)).wait()
  // add nft to marketplace
  await(await store.connect(deployer).makeItem(nft.address, id, collection, toWei(price))).wait();
